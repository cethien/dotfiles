...
ip route | grep -q "default" && HAS_INTERNET=1 || HAS_INTERNET=""
...
if [ -n "$HAS_TAILSCALE" ]; then
	...
	if [ "$TS_STATUS" = "NeedsLogin" ] && [ -n "$HAS_INTERNET" ]; then
		add_opt "󰍂 login to tailscale" "sudo tailscale login" "silent"
	elif [ "$TS_STATUS" = "Running" ]; then
		# Ausschalten geht immer, auch ohne Internet
		add_opt "󰌙 disable tailscale" "tailscale down" "silent"
	elif [ "$TS_STATUS" = "Stopped" ] && [ -n "$HAS_INTERNET" ]; then
		# Einschalten nur mit Internet sinnvoll
